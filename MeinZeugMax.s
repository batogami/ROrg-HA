...
# buffer durchgehen bis leerzeichen

load_img:
#open a file
    li 		$v0, 13    	#Syscall for loading files
    la 		$a0, bild  	#Saving the address to $a0
    li   	$a1, 0  	#flag for reading
    li   	$a2, 0  	#mode is ignored
    syscall
    move 	$s0, $v0 	#Placing the descriptor in $s6 for later-use  

#read from file
    li 		$v0, 14   	#Syscall to read from files
    move 	$a0, $s0   	#Moving descriptor to $a0
    la		$a1, buffer	#address of buffer to which to read
    li   	$a2, 65555 	#hardcoded buffer length
	syscall
	
#auslesen von length und height

	addi	$t0, $zero, 3 		#$t0 = i = 3 starte bei string durchlauf beim ersten Leerzeichen -> überspringe P5 beim lesen
	addi	$
	for: 
		bge $t0, $a0, endfor 
...
interpolate2:
	...
	la		$t0,	# Lädt das Array des Bildes
	
	#for-Schleife für das verkleinern um den Faktor 2
	add			$t0, $zero, $zero 		#$t0 = i = 0
	add 		$t1, $zero, $zero 		#$t1 = y = 0
	# $s0 ist die Höhe des Bildes
	# in $s1 wird die Breite des Bildes gespeichert
	li 			$s2, 0					#$s0 = 0
	addi		$s3, $s3, 1				#$s3 = 1
	addi		$s4, $s3, 1				#$s4 ist der "Punkt" unter $s0
	addi		$s5, $s4, 1				#$s5 ist der "Punkt" unter $s1
	for:
		bge		$t0, $s0, endfor		# IF ($t0 >= $s0) GOTO endfor
		
		#Befehle
		for:
			bge $t1, $s1, endfor		# IF ($t1 >= $s1) GOTO endfor
		...
		addi $s2, $s1, 2
		add  $s2, $s2, $s1	#a = width * 2 +1 -> neue Zeile 
		
		addi $s3, $s3, 1	#b = width * 2 + 2 -> neue Zeile
		
		add  $s4, $s4, $s1
		add	 $s4, $s4, $s1
		add	 $s4, $s4, $s1
		addi $s4, $s4, 3	#c = width * 3 + 3 -> neue Zeile
		
		add  $s5, $s4, 1	#d = width * 3 + 4 -> neue Zeile
